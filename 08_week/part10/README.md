...
이번 파트에서는 메서드 오버라이드, 프로토타입 체인, 다중 프로토타입 체인에 대해 알아봤는데요.
다중 프로토타입 체인에서는 '두 단계 이상의 체인을 지니는 다중 프로토타입 체인'도 가능하다는 사실을 확인했습니다. 다음 class 파트에서는 본격적으로 이렇게 하는 이유와 구현 방식 및 문제 해결 등에 대해 더 자세히 알아보겠습니다.